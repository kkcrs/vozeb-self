...
- [生成] 图片标准比例（1:1 / 9:16 / 16:9 / 3:4 / 4:3）始终映射到上游常见固定档位，不再因“高画质”推成 StepFun 等模型不支持的像素尺寸。
- [生成] MiniMax 视频任务创建成功后，查询路径 `?task_id=` 会正确附带任务 ID，并允许系统代理查询原上游任务，避免界面一直停在“已等待”。
- [后台] 删除或保存模型渠道时，不再把 MiniMax-H3 误判成缺少创建路径的文本自定义协议，避免删 StepFun 等无关渠道被拦住。
